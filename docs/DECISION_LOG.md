...
|---|---|---|
| 2026-08-13 | Added "Rà soát câu hỏi" on mock-exam result screen (list each question with ✅/❌, user answer, correct answer, explanation; filter-wrong toggle) | User reported exams did not show which answers were wrong or explain them; only score/module breakdown was shown. Reuses the same grading logic and the per-question `explanation` fields. |
| 2026-08-13 | Raised difficulty of Modules A–C across all 3 mock exams (kept A=15/B=21/C=15/D=9, 60 q / 90 min; Module D unchanged) | User felt content was too basic and wanted it more practical. Replaced one-liner "recall" questions with multi-step NumPy/Pandas/matrix questions (weighted mean, outlier/median, det, norm, np.linalg.solve, boolean mask, np.where, groupby/merge, axis reductions, dict/list comprehension) while keeping hand-computable small numbers. Every new question got an accurate explanation. |
